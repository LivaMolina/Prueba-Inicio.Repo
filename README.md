...
## CLASE 09 MIÉRCOLES 8 DE OCTUBRE DEL 2025
Configurar múltiples colaboradores en un repositorio de GitHub<br>
...
```sh
git commit —amend #Corregimos el mensaje, sirve para modificar el último commit que hiciste, sin crear uno nuevo.
git pull origin main #Traer el repositorio remoto
git push --set-upstream origin main #Ejecutar el cambio, el error arreglado
```
Ejemplo 1
```sh
git commit -m "Agregamos el arcivo principal"
git commit --amend -m "Agregamos el archivo principal" # Esto reemplaza el mensaje anterior sin tocar los archivos del commit.
```
Ejemplo 2: Agregar un archivo que olvidaste incluir
Hacés un commit:
```sh
git commit -m "Subimos los cambios del proyecto"
```
Te das cuenta de que te olvidaste de incluir index.html.
```sh
git add index.html
```
Ahora usás --amend para agregarlo al último commit:
```sh
git commit --amend -m "Subimos los cambios del proyecto con index.html"
```
Git reemplaza el commit anterior por uno nuevo que incluye el archivo olvidado y el nuevo mensaje<br>

⚠️ Importante:

Solo usá --amend en commits que todavía no hayas subido (push) al repositorio remoto.<br>
Si ya hiciste git push, lo ideal es no modificarlo, porque puede causar conflictos con otros colaboradores.<br>
